# 18-21
# Вместо предисловия
Копал несколько часов в инете по данным вопросам, просмотрел достаточное количество статей, но вынужден признать, что книга параллельное программирование отвечает на данные вопросы наиболее полно
# Пул потоков
...
Но данный пул потоков не поддерживает многие функции такие как ожидание задачи или предотвращение конкуренции потоков. Усложненные варианты написаны в следующих пунктах. Их можно посмотреть по диагонали, но разобраться чтобы потребуется несколько часов, поэтому запоминайте, что пул из листинга 9.1 очень не идеален, но реализация всех этих функций займет ОЧЕНЬ МНОГО времени
...
# Реализоация потокобезопасной очереди с блокировкой.
...
# Реализоация потокобезопасного стека с блокировкой.
